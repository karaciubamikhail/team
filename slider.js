...
class ItcSlider {
   ...
   * @param {String} prefix
   */
  constructor(el, config = {}, prefix = 'say_slider-') {
    this.#state = {
   ...
   * @param {String} prefix
   */
  static getOrCreateInstance(target, config = {}, prefix = 'say_slider-') {
  ...
  static createInstances() {
    document.querySelectorAll('[data-slider="say_slider"]').forEach((el) => {
      const { dataset } = el;
      const params = {};
  ...
  #onClick(e) {
    ...
    if (!(e.target.closest('.say_slider-btn') || e.target.closest('.say_slider-indicators'))) {
      return;
    }
